...
public class SysmeXTImplementation extends AnalyzerLineInserter {
  ...
  private void addAnalyzerResultFromLine(List<AnalyzerResults> results, String line) {
    ...
    // Timestamp timestamp = DateUtil.convertStringDateToTimestampWithPattern(fields[DATE] + " " +
    // fields[TIME], DATE_PATTERN);
    Timestamp timestamp =
    ...
    for (int i = 0; i < orderedTestIndexs.length; i++) {
      ...
      if (!GenericValidator.isBlankOrNull(testNameIndex[testIndex])) {
        MappedTestName mappedName =
            AnalyzerTestNameCache.getInstance().getMappedTest("SysmeXT", testNameIndex[testIndex]);

        if (mappedName == null) {
          mappedName =
              AnalyzerTestNameCache.getInstance()
                  .getEmptyMappedTestName("SysmeXT", testNameIndex[testIndex]);
        }
