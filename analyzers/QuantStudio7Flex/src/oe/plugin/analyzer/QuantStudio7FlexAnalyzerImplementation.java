 ...
 *
 * Extends the QuantStudio3 parsing logic to support additional columns present in QS7 Flex exports.
 *
 * Expected CSV columns (tab-delimited):
 * Well, Well Position, Sample Name, Target, Task, Reporter, Quencher, Amp Status, CT, Ct Mean, Ct SD, ...
 *
 * Key differences from QS3:
 * - "Well Position" column (e.g., "A1", "A2")
 * - "Target" column for test identification (e.g., "SARS-CoV-2", "IC")
 * - "Task" column (UNKNOWN, Control)
 * - "Reporter" and "Quencher" columns for probe info
 * - "Amp Status" column (Positive, Negative)
 *
...
public class QuantStudio7FlexAnalyzerImplementation extends AnalyzerLineInserter {
    ...
    public void addResultLine(
        ...
        String currentAccessionNumber = getColumnValue(resultData, "Sample Name").replace("\"", "").trim();
        ...
        LogEvent.logDebug(
                ...
                "addResultLine",
                "QS7Flex: " + analyzerResult.getAccessionNumber()
                        + " Target=" + target
                        + " CT=" + ctValue
                        + " AmpStatus=" + ampStatus);
    ...
    private String determineTargetLoinc(String target) {
        ...
        if (upperTarget.contains("IC") || upperTarget.contains("INTERNAL") || upperTarget.contains("CONTROL")) {
