...
public class WeberAnalyzerImplementation extends AnalyzerLineInserter {
    ...
    for (int i = 0; i < testNameIndex.length; i++) {
      if (testNameIndex[i] != null && testNameIndex[i].length() > 0) {
        MappedTestName mappedName =
            AnalyzerTestNameCache.getInstance().getMappedTest("WeberAnalyzer", testNameIndex[i]);

        if (mappedName == null) {
          mappedName =
              AnalyzerTestNameCache.getInstance()
                  .getEmptyMappedTestName("WeberAnalyzer", testNameIndex[i]);
        }
  ...
  public String getError() {
    return "Weber analyzer unable to write to database";
  }
}
