    ...
    for (int i = 0; i < testNameIndex.length; i++) {
      if (testNameIndex[i] != null && testNameIndex[i].length() > 0) {
        MappedTestName mappedName =
            AnalyzerTestNameCache.getInstance().getMappedTest("FacsCalibur", testNameIndex[i]);

        if (mappedName == null) {
          mappedName =
              AnalyzerTestNameCache.getInstance()
                  .getEmptyMappedTestName("FacsCalibur", testNameIndex[i]);
        }
        ...
        // ------------------------------------------------
        if (analyzerAccessionNumber.contains("CHRA")
            || analyzerAccessionNumber.contains("IL")
            || analyzerAccessionNumber.contains("IC")
            || analyzerAccessionNumber.contains("QC-")) {
