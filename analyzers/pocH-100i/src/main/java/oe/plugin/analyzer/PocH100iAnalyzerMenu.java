...
public class PocH100iAnalyzerMenu extends MenuPlugin {
  ...
  protected void insertMenu() {
    ...
    menu.setActionURL("/AnalyzerResults?type=" + PocH100iAnalyzer.ANALYZER_NAME);
    // The key used for the name of the analyzer on the menu. Should not already
    // exist in MessageResource.properties.
    menu.setDisplayKey("banner.menu.results." + PocH100iAnalyzer.ANALYZER_NAME);
    menu.setOpenInNewWindow(false);

    service.addMenu(menu);
    // Analyzer name in English
    service.insertLanguageKeyValue(
        "banner.menu.results." + PocH100iAnalyzer.ANALYZER_NAME,
        PocH100iAnalyzer.ANALYZER_NAME,
